...
export const getWeekForecastWeather = (response, descriptions_list) => {
  ...
  forecast_keys.forEach((key, idx) => {
    ...
    for (let i = 0; i < grouped_forecast_data[key].length; i++) {
      ...
      dayminTempsList.push(grouped_forecast_data[key][i].mintemp);
      daymaxTempsList.push(grouped_forecast_data[key][i].maxtemp);
      dayPresList.push(grouped_forecast_data[key][i].pressure);
    }
    ...
    dayAvgsList.push({
      ...
      mintemp: getMinimum(dayminTempsList),
      maxtemp: getMaximum(daymaxTempsList),
      pressure: getAverage(dayPresList, false),
...
export const getTodayForecastWeather = (
  ...
  if (all_today_forecasts.length < 6) {
    return [...all_today_forecasts];
  } else {
    return all_today_forecasts.slice(-5);
  }
};
